 ...
 *
 * This file is read by the framework mission setup UI, mission manager, and
 * built-in task generators.
 *
 * Startup UI behavior:
 * - Arma mission params/defaults provide the framework setup UI defaults.
 * - If the setup UI is cancelled, those same params/defaults are applied by
 *   the framework mission setup service.
 ...
 *   forge_server_task_missionSetup_settings by the framework setup service.
 * - Reward, reputation, penalty, and timeLimit ranges are read through
 *   forge_server_task_fnc_getMissionSettingRange so UI overrides and config
 *   fallbacks use the same path.
 */
class CfgMissions {
